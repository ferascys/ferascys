# Firas Al-Sharaidi
**Cybersecurity Student | Tech Enthusiast | Innovator**

Welcome to my GitHub profile. I am a Cybersecurity student at **King Saud University (KSU)** with a strong foundation in network defense, embedded systems, and data-driven security. I am passionate about building secure infrastructures and developing innovative technological solutions.

### 🛡️ Professional Overview
- 🎓 **Academics:** Pursuing a degree in Cybersecurity at KSU.
- 🏛️ **Leadership:** Active member of the Public Relations Committee at the **KSU Cybersecurity Club**.
- 🚀 **Certifications path:** Currently advancing my technical expertise by preparing for **Security+, Network+, Linux+, and eJPTv2**.
- 💡 **Innovation:** Actively participating in national tech initiatives, including the **Saef Innovation Competition** under the Global Summit for Safety and Technology.

### 💻 Featured Projects & Experience
- **Stock Market Projection System:** Collaborated on a faculty-supervised academic prototype that merges data analytics with cybersecurity principles to project stock market trading timelines.
- **Embedded Systems & Hardware:** Building and configuring physical circuit prototypes using **Arduino** and microcontrollers to understand hardware-level security and logic.
- **CyberGuard Bootcamp:** Successfully completed an intensive foundational cybersecurity training camp.

### ⚙️ Core Competencies & Interests
- **Security:** Penetration Testing, Network Defense, Systems Security.
- **Hardware/IoT:** Microcontrollers, Circuit Design, Arduino.
- **Research:** Intersections of data analytics, financial forecasting, and cyber defense.

### 📫 Connect With Me
http://linkedin.com/in/feras-alsharedi
